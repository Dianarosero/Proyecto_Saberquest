...
// Helper para usar SweetAlert2 si está disponible
...
function filterTable(tableId, column, query) {
  ...
  const pagination = document.getElementById(paginationId);
  if (pagination) {
    const firstPageBtn = pagination.querySelector(
...
function validateEditForm(e) {
  ...
  if (newPassword && confirmPassword && newPassword.value) {
    if (newPassword.value !== confirmPassword.value) {
      swalWarn("Las contraseñas no coinciden.");
      isValid = false;
    }
  }
  ...
  if (
    ...
    swalWarn(
      "Debe proporcionar la contraseña actual para cambiar la contraseña."
    );
    isValid = false;
  }
