...
<body>
  <div class="container">
    ...
    <div class="illustration-column">
      <div class="illustration-content">
        <img src="/src/assets/illustration.svg" class="illustration" alt="Ilustración educativa">
        <div class="illustration-text">
          <h2>Educación y Aprendizaje</h2>
          <p>Únete a nuestra plataforma educativa y accede a recursos, cursos y herramientas diseñadas para tu éxito
            académico.</p>
        </div>
      </div>
    </div>
      ...
      <div class="form-container">
        ...
        <form id="registration-form" action="crear_usuario.php" method="post" novalidate>
          <div class="form-group">
            ...
            <input type="text" id="fullName" name="nombres" placeholder="Ingresa tu nombre completo" required
          ...
          <div class="form-group">
            ...
            <input type="text" id="studentCode" name="num_identificacion" placeholder="Ingresa tu código institucional" required
              oninput="this.value = this.value.toUpperCase()" style="text-transform: uppercase;" />
          ...
          <div class="form-group">
            ...
            <div class="password-container">
              <input type="password" id="password" name="contraseña" placeholder="Crea una contraseña" required style="text-transform: none;" />
              <button type="button" id="toggle-password" aria-label="Mostrar/ocultar contraseña">
                <svg class="eye-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20"
  ...
  <script>
    function handleUserTypeChange() {
      ...
      if (userType === 'student' || userType === 'ESTUDIANTE') {
    ...
    document.getElementById('registration-form').addEventListener('submit', function (e) {
      ...
      if ((userType === 'student' || userType === 'ESTUDIANTE') && !semester) {
  ...
  <script>
    ...
    const semesterSelect = document.getElementById('semester'); // Asegúrate que exista este elemento
    const submitButton = document.getElementById('submitButton');

    // Funciones de validación (debes tenerlas definidas)
    ...
    function validateStudentCode(value) {
      if (!value.trim()) return 'El código de estudiante es obligatorio';
      // Más validaciones si quieres...
      return '';
    }
    ...
    form.addEventListener('submit', function (e) {
      ...
      // Validar semestre solo si es estudiante
      let semesterError = '';
      if (userTypeSelect.value === 'ESTUDIANTE' || userTypeSelect.value === 'student') {
        if (!semesterSelect.value) {
          semesterError = 'Selecciona el semestre';
        }
      }
      ...
      // Si hay algún error, bloquear el envío
      if (fullNameError || studentCodeError || passwordError || userTypeError || semesterError) {
        e.preventDefault();
      }
      // Si no hay errores, el formulario se enviará automáticamente al archivo PHP definido en el atributo action del <form>
    });

    // Accesibilidad: gestión de enfoque
    const formElements = form.querySelectorAll('input, select, button');
    formElements.forEach(element => {
      element.addEventListener('keydown', (e) => {
        // Si se presiona la tecla Tab sin shift
        if (e.key === 'Tab' && !e.shiftKey) {
          const lastElement = formElements[formElements.length - 1];
          if (element === lastElement) {
            // Prevenir comportamiento predeterminado de tab
            e.preventDefault();
            // Mover el enfoque al primer elemento
